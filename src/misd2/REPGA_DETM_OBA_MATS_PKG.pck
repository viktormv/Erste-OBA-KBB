CREATE OR REPLACE PACKAGE REPGA_DETM_OBA_MATS_PKG IS
	-- %version   0.2   2012.12.10.
	-- %version   0.1   2012.10.25.
	-- %author   Dörnyei László
	-- %usage loading package for detail area
	-- %intab tablelist:
	--                    CTFK_SYS.CT_LOAN_MTH<BR/>
	--                    CTFK_SYS.FK_CCY<BR/>
	--                    CTFK_SYS.FK_SAP_TRAN<BR/>
	--                    KMDW.MI_RB_ACCT_MTH<BR/>
	--                    KMDW.MI_RB_ACCT_DETAIL_MTH<BR/>
	--                    KMDW.MI_RB_OD_FACILITY_MTH<BR/>
	--                    KMDW_EX_ODS.ODS_EX_FK_BALANCE<BR/>
	--                    KMDW.MI_CL_INVOICE_MTH<BR/>
	-- %outtab tablelist: REPGA_DETM_OBA_MATS

	PROCEDURE MAIN(P_SYM_RUN_DATE DATE DEFAULT EBH_PWM_PUBLIC_P.F_GETSYMRUNDATE);

END;
/
CREATE OR REPLACE PACKAGE BODY REPGA_DETM_OBA_MATS_PKG IS
	-- %version   0.2   2012.12.10.
	-- %version   0.1   2012.10.25.
	-- %author   Dörnyei László
	--
	-- Procedure for calculating and loading data into REPGA_DETM_OBA_MATS table
	...
	-- %dbmsout NO
	----------------------------------------------------------------------------------
	-- %version   0.2   DL ERSTEOBA-3 javítópatch
	-- %version   0.1   DL ERSTEOBA-3 INITIAL

	GC_APPNAME CONSTANT VARCHAR2(30) := 'REPGA_DETM_OBA_MATS_PKG';

	GC_TARGETTABLE CONSTANT VARCHAR2(30) := 'REPGA_DETM_OBA_MATS';
	...
	PROCEDURE INSERT_TABLE(P_SYM_RUN_DATE DATE) IS
		...
		TYPE T_SYM_RUN_DATE IS TABLE OF REPGA_DETM_OBA_MATS.SYM_RUN_DATE%TYPE INDEX BY PLS_INTEGER;
		TYPE T_CLIENT_NO IS TABLE OF REPGA_DETM_OBA_MATS.CLIENT_NO%TYPE INDEX BY PLS_INTEGER;
		TYPE T_AMT IS TABLE OF REPGA_DETM_OBA_MATS.AMT%TYPE INDEX BY PLS_INTEGER;
		--TYPE T_RUN_ID IS TABLE OF REPGA_DETM_OBA_MATS.RUN_ID%TYPE INDEX BY PLS_INTEGER;
		...
		CURSOR HC IS(
			SELECT SYM_RUN_DATE,
						 CLIENT_NO,
						 SUM(AMOUNT_LCY) AMT
				FROM REPGA_DETM_EXPIRED_AMTS
			 WHERE SYM_RUN_DATE = P_SYM_RUN_DATE --(SELECT actdat FROM OBA_DATE)
			 -->v0.2 GROUP BY CLIENT_NO);
       GROUP BY SYM_RUN_DATE, CLIENT_NO);--<v0.2
	BEGIN
		REPGA_BACKOFFICE_PKG.API_ACTION(C_ACCTION);
		OPEN HC;
		LOOP
			...
			FETCH HC BULK COLLECT
				INTO L_SYM_RUN_DATE, L_CLIENT_NO, L_AMT LIMIT G_BATCHSIZE;
			L_LAST := L_SYM_RUN_DATE.COUNT;
			EXIT WHEN L_LAST = 0;
			L_ROWNUMBER := L_ROWNUMBER + L_LAST;

			FORALL X IN 1 .. L_LAST
				INSERT INTO REPGA_DETM_OBA_MATS
					(SYM_RUN_DATE,
					 CLIENT_NO,
					 AMT,
					 RUN_ID)
