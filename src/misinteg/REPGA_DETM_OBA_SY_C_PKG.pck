CREATE OR REPLACE PACKAGE REPGA_DETM_OBA_SY_C_PKG IS
	-- %version   0.3   2013.04.15.
	-- %version   0.2   2012.12.17.
...
CREATE OR REPLACE PACKAGE BODY REPGA_DETM_OBA_SY_C_PKG IS
	...
	-- %dbmsout NO
	----------------------------------------------------------------------------------
	-- %version   0.3   DL ERSTEOBA-3 REPGA_TMPM_OBA_CONS_M töltés performanciajavítás
	-- %version   0.2   DL ERSTEOBA-3 P_SYM_RUN_DATE kiegészítés
	...
	BEGIN
       ...
       SELECT a.CLIENT_NO,
							...
							a.MMN,
							a.BIRTH_DATE
				 FROM REPGA_TMPM_OBA_CONS a,
         (SELECT MN|| MMN || TO_CHAR(BIRTH_DATE,'YYYY.MM.DD.') felt
																					FROM REPGA_TMPM_OBA_CONS
																				 WHERE CONS_BIT = 1
																				 GROUP BY MN|| MMN || TO_CHAR(BIRTH_DATE,'YYYY.MM.DD.')
																				HAVING COUNT(1) > 1) b
       				WHERE a.CONS_BIT = 1
					AND a.MN|| a.MMN || TO_CHAR(a.BIRTH_DATE,'YYYY.MM.DD.') = b.felt);
		COMMIT;
			...
			(SELECT CLIENT_NO,
							(SELECT REPLACE(ADDRESS,
								...
								WHERE CLIENT_NO = X.CLIENT_NO
									...
									AND SYM_RUN_DATE = P_SYM_RUN_DATE) PHONE_TEL, --v0.2
							(SELECT REPLACE(ADDRESS,
															';',
															'')
								 FROM MI_FM_CLIENT_CONTACT_MTH
								WHERE CLIENT_NO = X.CLIENT_NO
									...
									AND SYM_RUN_DATE = P_SYM_RUN_DATE) PHONE_MOB,--v0.2
							(SELECT REPLACE(CONTACT_ID,
								...
								WHERE CLIENT_NO = X.CLIENT_NO
									...
									AND SYM_RUN_DATE = P_SYM_RUN_DATE) E_MAIL--v0.2
