CREATE OR REPLACE PACKAGE REPGA_DETM_OBA_RES_DOLG_BM_PKG IS
	...
	--
	-- %outtab tablelist: REPGA_DETM_OBA_RES_DOLG_BM
	PROCEDURE MAIN(P_SYM_RUN_DATE DATE DEFAULT EBH_PWM_PUBLIC_P.F_GETSYMRUNDATE);

END;
/
CREATE OR REPLACE PACKAGE BODY REPGA_DETM_OBA_RES_DOLG_BM_PKG IS
	-- %version   0.1   2012.10.11
	-- %author   Dörnyei László

	-- Procedure for calculating and loading data into REPGA_DETM_OBA_RES_DOLG_BM table
	...
	GC_APPNAME CONSTANT VARCHAR2(30) := 'REPGA_DETM_OBA_RES_DOLG_BM_PKG';

	GC_TARGETTABLE CONSTANT VARCHAR2(30) := 'REPGA_DETM_OBA_RES_DOLG_BM';
	...
	PROCEDURE INSERT_TABLE(P_SYM_RUN_DATE DATE) IS
		...
		--TYPE T_SYM_RUN_DATE IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.SYM_RUN_DATE%TYPE INDEX BY PLS_INTEGER;
		TYPE T_CLIENT_NO IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.CLIENT_NO%TYPE INDEX BY PLS_INTEGER;
		TYPE T_ACCOUNT_NO IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.ACCOUNT_NO%TYPE INDEX BY PLS_INTEGER;
		TYPE T_UA IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.UA%TYPE INDEX BY PLS_INTEGER;
		TYPE T_TIPUS IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.TIPUS%TYPE INDEX BY PLS_INTEGER;
		TYPE T_BJOGCIM IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.BJOGCIM%TYPE INDEX BY PLS_INTEGER;
		TYPE T_BAL_AMT IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.BAL_AMT%TYPE INDEX BY PLS_INTEGER;
		TYPE T_BAL_HUF IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.BAL_HUF%TYPE INDEX BY PLS_INTEGER;
		TYPE T_INT_HUF IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.INT_HUF%TYPE INDEX BY PLS_INTEGER;
		TYPE T_INT_TAX IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.INT_TAX%TYPE INDEX BY PLS_INTEGER;
		TYPE T_RESTRAINT IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.RESTRAINT%TYPE INDEX BY PLS_INTEGER;
		--TYPE T_RUN_ID IS TABLE OF REPGA_DETM_OBA_RES_DOLG_BM.RUN_ID%TYPE INDEX BY PLS_INTEGER;
	...
	BEGIN
		...
		LOOP
			...
			FORALL X IN 1 .. L_LAST
				INSERT INTO REPGA_DETM_OBA_RES_DOLG_BM
					(SYM_RUN_DATE,
					 CLIENT_NO,
					 ACCOUNT_NO,
					 UA,
